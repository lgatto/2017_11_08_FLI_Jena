...
\begin{frame}{Spatial proteomics - Why?}
  ...
  \begin{block}{Re-localisation in} 
    ...
    \item \textcolor{Blue}{Activation} of biological processed.
    \end{itemize}
    Examples later.
  \end{block}  
\end{frame}
...
\begin{frame}{Novelty detection}
  \begin{figure}
    ...
    \caption{Left: Original \textit{Drosophila} data from
      \cite{Tan2009}. Right: After semi-supervised learning,
      \cite{Breckels:2013}.}
  \end{figure} 
\end{frame}
...
\begin{frame}{Computational advances: Transfer learning}
  What about using addition data data from repositories such as the
  Gene Ontogy (GO), sequence features (pseudo-aminoacid composition),
  signal peptide, transmembrane domains (length, number, ...), images
  (IF, FP), prediction software, \ldots
...
\begin{frame}
  ...
  (left). Example from embryonic stem cells \citep{Christoforou:2016}
  (right).
  \begin{columns}
